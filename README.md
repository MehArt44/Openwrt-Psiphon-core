...
این پروژه یک راهنمای جامع و اسکریپت هوشمند برای راه‌اندازی، مدیریت و جفت‌کردن هسته لینوکسی سایفون (`psiphon-core`) با روترهای مبتنی بر **OpenWrt** (به ویژه نسخه 24.10 و پردازنده‌های ۶۴ بیتی با معماری `aarch64_cortex-a53` و 
...
# ۳. دانلود وابستگی‌ها و تنظیم متغیرهای محیطی برای لینوکس ۳۲ بیتی ARM (مناسب برای پردازنده روتر شما)
go mod download
export GOOS=linux
export GOARCH=arm
export GOARM=7

# ۴. کامپایل فوق‌العاده بهینه، بدون حجم اضافی دباگ و سبک برای روتر
go build -ldflags="-s -w" -o psiphon-core .
